    ...
    <section ref={sectionRef} className="py-12 md:py-18 bg-[#F5F0E8] text-[#2C2C2C] overflow-hidden">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12">
        ...
        <div ref={headerRef} className="reveal-init mb-16 md:mb-20 lg:mb-24">
          <h2 className="text-4xl md:text-5xl lg:text-7xl mb-4 md:mb-6 text-[#2C2C2C]">
        ...
        <div className="block lg:hidden space-y-12">
          {blogs.map((blog, idx) => (
            <div key={idx} className="bg-white/40 backdrop-blur-sm rounded-3xl p-6 border border-[#9B8B7A]/10">
              ...
              {/* Title */}
              <h3 className="text-xl text-[#2C2C2C] mb-3 leading-tight">
                {blog.title}
              </h3>
              
              {/* Description */}
              <p className="text-[#5F5B57] text-sm mb-6 leading-relaxed">
                {blog.description}
              </p>
              
              {/* Button */}
              <Link to={`/blog/${blog.slug}`}>
                <button className="group flex items-center gap-3 text-xs tracking-widest uppercase py-2 border-b border-[#9B8B7A]/30 hover:border-[#E6B8A2] transition-colors text-[#2C2C2C]">
                  Read Article
          ...
          <div className="lg:col-span-5 z-20 flex flex-col justify-center pr-8 lg:pr-12">
            <div className="overflow-hidden h-[320px] relative">
              {blogs.map((blog, idx) => (
                ...
                >
                  <h3 className="text-2xl md:text-3xl lg:text-4xl leading-tight mb-4 text-[#2C2C2C] max-w-md">
                    {blog.title}
                  </h3>
                  <p className="text-[#5F5B57] text-base md:text-lg mb-6 max-w-md leading-relaxed">
                    {blog.description}
                  </p>
                  <Link to={`/blog/${blog.slug}`}>
                    <button className="group flex items-center gap-4 text-xs tracking-widest uppercase py-3 border-b border-[#9B8B7A]/30 hover:border-[#E6B8A2] transition-colors text-[#2C2C2C]">
                      Read Article
                      <span className="group-hover:translate-x-2 transition-transform duration-300 text-[#9B8B7A]">→</span>
                    </button>
                  </Link>
